...
public class DriverService {
  ...
  private static final String MATCH_FOUND_KAFKA_DEDUP_KEY_PREFIX =
      "match_found_processed_kafka_msg:";
  ...
  public boolean processDriverInfo(
      Long driverId, List<String> routePlaces, String finalDestination, Integer availableSeats) {
    // Try to acquire lock
    ...
    if (lockValue == null) {
      log.error(
          "Unable to acquire lock with retry policy: {} lock key {} timeout milliseconds {} maximum"
              + " retries {} back off milliseconds. Returning false",
          redisDriverLockKey,
          5000,
          10,
          200);
      ...
      log.info(
          "Acquired lock with retry policy: {} lock key {} timeout milliseconds {} maximum retries"
              + " {} back off milliseconds. Returning false",
          redisDriverLockKey,
          5000,
          10,
          200);
    ...
    try {
      ...
      if (driverId == null
          ...
          || availableSeats <= 0) {
        log.warn(
            "Invalid driver info: id={}, routeSize={}, dest={}, seats={}. Returning false.",
            driverId,
            routePlaces == null ? null : routePlaces.size(),
            finalDestination,
            availableSeats);
        return false;
      }
      ...
      log.info(
          "Reached DriverService.processDriverInfo with driverId = {}, routePlaces = {},"
              + " finalDestination = {}, availableSeats = {}",
          driverId,
          routePlaces,
          finalDestination,
          availableSeats);
          ...
          DriverCache.builder()
              ...
              .timeToNextPlace(
                  computeDurationFromDistance(
                      getDistanceBetween(
                          routePlaces.get(0), routePlaces.get(1), locationLocationMap)))
              .distanceToNextPlace(
      ...
      log.error(
          "Failed to process driver info for ID {}: {}. Returning false.",
          driverId,
          e.getMessage());
      return false;
    } finally {
  ...
  public void matchFoundUpdateCache(byte[] message, Acknowledgment acknowledgment) {
    // Try to acquire lock
    String lockValue = tryAcquireLockWithRetry(redisDriverLockKey);
    if (lockValue == null) {
      log.error(
    ...
    try {
      ...
      if (alreadyProcessed(MATCH_FOUND_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "DriverService.matchFoundUpdateCache: Duplicate Kafka message detected. Skipping."
                + " messageId={}",
            messageId);
        acknowledgment.acknowledge();
        return;
      ...
      if (driverCache == null) {
        log.warn(
            "Driver with ID = {} not found in cache during match update. Returning void.",
            driverId);
        return;
      }
  ...
  public void cronJobDriverLocationSimulation() {
    ...
    try {
      ...
      for (Map.Entry<Long, DriverCache> e : allDriverCacheData.entrySet()) {
        ...
        try {
          boolean evict =
              processSingleDriverTick(driverId, cache, locationLocationMap, nearbyStationMap);
          if (evict) {
      ...
      log.debug(
          "cron tick - driver simulation finished. updated drivers: {}, evicted: {}",
          allDriverCacheData.size(),
          driversToEvict.size());
    } catch (Exception e) {
   ...
   * compute next metro station and emit Kafka event. // * @param driverId
   *
  ...
  private boolean processSingleDriverTick(
    ...
    // find index of currentNextPlace in route (we assume route list is ordered in traversal
    // direction)
    ...
    if (newDistanceToNext > 0) {
      ...
      while (true) {
        ...
        if (prevPlace != null && prevPlace.equals(cache.getFinalDestination())) {
          ...
          log.info(
              "Driver location: Driver reached final destination: driverId={}, finalDest={}",
              driverId,
              prevPlace);
          DriverRideCompletionEvent event =
              DriverRideCompletionEvent.newBuilder()
          ...
          CompletableFuture<SendResult<String, byte[]>> future =
              kafkaTemplate.send(
                  RIDE_COMPLETION_TOPIC, String.valueOf(driverId), event.toByteArray());
          future
              .thenAccept(
                  result -> {
                    log.debug(
        ...
        if (Double.isInfinite(segmentDistance)) {
          ...
          log.warn(
              "Distance between {} and {} unknown. Stopping advancement for driver {}",
              prevPlace,
              nextPlace,
              driverId);
        ...
        if (remainder < segmentDistance) {
          ...
          if (remainder == 0) {
            ...
            double nextSegDistance =
                getDistanceBetween(prevPlace, followingPlace, locationLocationMap);
    ...
    // Update last seen / next metro station using nearbyStationMap scanning remaining route from
    // current position
    String lastSeen = Optional.ofNullable(cache.getLastSeenMetroStation()).orElse("");
    newNextMetroStation =
        findNextMetroStationInRoute(cache.getNextPlace(), cache.getRoutePlaces(), nearbyStationMap);
    ...
    // Simple heuristic: if oldMetroStation is empty and the nearest station for current nextPlace
    // equals something, set lastSeen accordingly.
    // For simulation robustness we'll update lastSeen only if we detect that we passed a node with
    // a station.
    String passedStation =
        detectPassedMetroStationDuringTick(
            cache, locationLocationMap, nearbyStationMap, DISTANCE_PER_TICK);
    ...
    if (cache.getTimeToNextPlace() != null) {
      log.info(
          "Driver {}: Time to next route place ({}): {} seconds",
          driverId,
          cache.getNextPlace(),
          cache.getTimeToNextPlace().getSeconds());
    }
    ...
    int timeToNextStationSec =
        computeTimeToNextStationSec(
            cache, newNextMetroStation, locationLocationMap, nearbyStationMap);

    // If next station is same as old station (consecutive nodes nearby same station), time is 0
    if (nextStationForEvent != null
        && !nextStationForEvent.isEmpty()
        && nextStationForEvent.equals(oldStationForEvent)) {
    ...
    if (nextStationForEvent != null && !nextStationForEvent.isEmpty()) {
      if (availableSeats > 0) {
        ...
        DriverLocationEvent event =
            DriverLocationEvent.newBuilder()
                .setMessageId(UUID.randomUUID().toString())
                .setDriverId(driverId)
        ...
        log.debug(
            "Published driver location event for driver {}: oldStation={}, nextStation={}, tts={}s",
            driverId,
            oldStationForEvent,
            nextStationForEvent,
      ...
      log.debug(
          "Skipping Kafka event for driver {}: no next metro station found in remaining route.",
          driverId);
    }
  ...
  private Map<String, Map<String, Double>> safeReadLocationMap() {
    ...
    if (lockValue == null) {
      log.error(
          "Unable to acquire lock with retry policy: {} lock key {} timeout milliseconds {} maximum"
              + " retries {} back off milliseconds. Returning false",
          redisLocationLocationMapLockKey,
          5000,
          10,
          200);
    } else {
  ...
  /**
   * Get distance between two places using the locationLocationMap structure. Tries symmetric
   * lookup: map[a].get(b) or map[b].get(a). Returns Double.POSITIVE_INFINITY if unknown.
   */
  private double getDistanceBetween(
      String a, String b, Map<String, Map<String, Double>> locationLocationMap) {
  ...
  /**
   * Convert distance units to java.time.Duration (approx based on DISTANCE_PER_TICK). Returns
   * Duration in seconds computed as ceil(distance / DISTANCE_PER_TICK * SECONDS_PER_TICK).
   */
  ...
  /**
   * Find the first metro station mapped by nearbyStationMap in route starting from currentNextPlace
   * index inclusive. nearbyStationMap maps placeId -> stationId (if place has a nearby metro
   * station). Returns stationId or empty string if not found.
   */
  private String findNextMetroStationInRoute(
      String currentNextPlace, List<String> routePlaces, Map<String, String> nearbyStationMap) {
  ...
  /**
   * Detect if the driver passed a metro station during this tick. Simple heuristic: check nodes in
   * route that are within DISTANCE_PER_TICK from current progress. For simulation we return the
   * first matched station id, else empty.
   */
  private String detectPassedMetroStationDuringTick(
      DriverCache cache,
      Map<String, Map<String, Double>> locationLocationMap,
      Map<String, String> nearbyStationMap,
  ...
  /**
   * Compute time to next metro station in seconds from current driver state using map distances. If
   * nextStationId is empty string -> return 0 (or -1 optionally). We return 0 per instructions.
   */
  private int computeTimeToNextStationSec(
      DriverCache cache,
  ...
  /**
   * Normalize the Redis 'drivers' value to a Map<Long, DriverCache> regardless of stored key/value
   * shapes. Accepts keys as Long/Integer/String and values as DriverCache or Map (convertible to
   * DriverCache).
   */
