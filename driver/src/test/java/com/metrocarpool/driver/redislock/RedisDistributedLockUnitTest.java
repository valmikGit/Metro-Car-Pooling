...
class RedisDistributedLockUnitTest {
  ...
  void acquireLock_Success() {
    ...
    when(valueOperations.setIfAbsent(
            eq(lockKey), anyString(), eq(timeout), eq(TimeUnit.MILLISECONDS)))
        .thenReturn(true);
    ...
    verify(valueOperations)
        .setIfAbsent(eq(lockKey), eq(lockValue), eq(timeout), eq(TimeUnit.MILLISECONDS));
  }
  ...
  void acquireLock_FailsWhenLockAlreadyHeld() {
    ...
    verify(valueOperations)
        .setIfAbsent(eq(lockKey), anyString(), eq(timeout), eq(TimeUnit.MILLISECONDS));
  }
  ...
  void acquireLock_UsesCorrectTimeout() {
    ...
    // Then
    verify(valueOperations)
        .setIfAbsent(eq(lockKey), anyString(), timeoutCaptor.capture(), timeUnitCaptor.capture());
