...
class DriverServiceUnitTest {
  ...
  @Mock private RedisTemplate<String, Object> redisTemplateNearby;

  @Mock private RedisTemplate<String, Object> redisTemplateLocationMap;

  @Mock private RedisDistributedLock redisDistributedLock;

  @Mock private RedisTemplate<String, String> redisStringTemplate;
  ...
  void processDriverInfo_Success() throws Exception {
    ...
    // When
    boolean result =
        driverService.processDriverInfo(driverId, routePlaces, finalDestination, availableSeats);
  ...
  void matchFoundUpdateCache_Success() throws Exception {
    ...
    DriverRiderMatchEvent event =
        TestDataBuilder.buildDriverRiderMatchEvent(driverId, riderId, pickupStation);
    ...
    verify(stringValueOperations)
        .set(contains("match_found_processed_kafka_msg"), eq("1"), eq(24L), any());
  ...
  void matchFoundUpdateCache_ShouldNotGoBelowZero() throws Exception {
    ...
    Long driverId = 1L;
    DriverRiderMatchEvent event =
        TestDataBuilder.buildDriverRiderMatchEvent(driverId, 100L, "Station1");
    byte[] message = event.toByteArray();
  ...
  void matchFoundUpdateCache_ShouldHandleMissingDriver() throws Exception {
    // Given
    DriverRiderMatchEvent event =
        TestDataBuilder.buildDriverRiderMatchEvent(999L, 100L, "Station1");
    byte[] message = event.toByteArray();
