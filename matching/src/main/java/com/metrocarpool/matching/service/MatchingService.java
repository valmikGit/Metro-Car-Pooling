...
public class MatchingService {
  ...
  private static final String DRIVER_UPDATE_KAFKA_DEDUP_KEY_PREFIX =
      "driver_update_processed_kafka_msg:";
  private static final String RIDER_REQUEST_KAFKA_DEDUP_KEY_PREFIX =
      "rider_request_processed_kafka_msg:";
  ...
  private HashMap<String, HashMap<String, List<MatchingDriverCache>>> ensureAllMatchingCache() {
    HashMap<String, HashMap<String, List<MatchingDriverCache>>> allMatchingCache =
        (HashMap<String, HashMap<String, List<MatchingDriverCache>>>)
            redisDriverTemplate.opsForValue().get(MATCHING_DRIVER_CACHE_KEY);
  ...
  /**
   * Get or create the station map for a station key. Returns the station map (String ->
   * List<MatchingDriverCache>) and ensures it is put in allMatchingCache.
   */
  ...
  private List<MatchingDriverCache> getOrCreateDriverList(
      HashMap<String, List<MatchingDriverCache>> stationMap, String destinationKey) {
  ...
  private HashMap<String, HashMap<String, Integer>> ensureDistancesMap() {
    HashMap<String, HashMap<String, Integer>> distances =
        (HashMap<String, HashMap<String, Integer>>)
            redisDistancesHashMap.opsForValue().get(MATCHING_DISTANCE_KEY);
    if (distances == null) {
  ...
  public void driverInfoUpdateCache(byte[] message, Acknowledgment ack) {
    ...
    try {
      ...
      if (alreadyProcessed(DRIVER_UPDATE_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "MatchingService.driverInfoUpdateCache: Duplicate Kafka message detected. Skipping."
                + " messageId={}",
            messageId);
        ack.acknowledge();
        return;
      ...
      if (oldStation != null && !oldStation.isEmpty()) {
        ...
        if (matchingCache != null && finalDestination != null && !finalDestination.isEmpty()) {
          ...
          if (matchingDriverCacheList != null) {
            matchingDriverCacheList.removeIf(
                matchingDriverCache1 ->
                    Objects.equals(matchingDriverCache1.getDriverId(), driverId));
      ...
      if (nextStation != null
        ...
        HashMap<String, List<MatchingDriverCache>> matchingCache1 =
            allMatchingCache.get(nextStation);
        ...
        matchingDriverCacheList1.add(
            MatchingDriverCache.builder()
                .driverId(driverId)
        ...
        log.info(
            "Skipping cache update for driver {}: nextStation or finalDestination is empty."
                + " nextStation={}, finalDestination={}",
            driverId,
            nextStation,
            finalDestination);
      }
  ...
  @KafkaListener(
      topics = "${kafka.topics.rider-requests}",
      groupId = "${spring.kafka.consumer.group-id}")
  public void riderInfoDriverMatchingAlgorithm(byte[] message, Acknowledgment acknowledgment) {
    // Try to acquire lock
    ...
    try {
      ...
      if (alreadyProcessed(RIDER_REQUEST_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "MatchingService.riderInfoDriverMatchingALgorithm: Duplicate Kafka message detected."
                + " Skipping. messageId={}",
            messageId);
      ...
      PriorityQueue<MatchingDriverCache> pq =
          new PriorityQueue<>(
              Comparator.comparingLong(
                  d ->
                      d.getTimeToReachStation() == null
                          ? Long.MAX_VALUE
                          : d.getTimeToReachStation().toMillis()));
                    ...
                    if (driverCache.getTimeToReachStation() != null) {
                      driverArrivalMillis =
                          System.currentTimeMillis()
                              + driverCache.getTimeToReachStation().toMillis();
                    }
                  } catch (Exception ex) {
          ...
          if (!pq.isEmpty()) {
            ...
            if (chosenDriver != null) {
              ...
              if (chosenDriver.getTimeToReachStation() != null) {
                driverArrivalMillis =
                    System.currentTimeMillis() + chosenDriver.getTimeToReachStation().toMillis();
              }
              Timestamp driverArrivalTs = Timestamps.fromMillis(driverArrivalMillis);

              DriverRiderMatchEvent event =
                  DriverRiderMatchEvent.newBuilder()
              ...
              log.info(
                  "Matching: Rider = {} and driver = {} matched.",
                  riderId,
                  chosenDriver.getDriverId());
              ...
              if (chosenDriverStation != null && chosenDriverDestination != null) {
                HashMap<String, List<MatchingDriverCache>> stationMap2 =
                    allMatchingCache.get(chosenDriverStation);
                if (stationMap2 != null) {
                  List<MatchingDriverCache> driverList = stationMap2.get(chosenDriverDestination);
                  if (driverList != null) {
                    driverList.removeIf(
                        mdc -> Objects.equals(mdc.getDriverId(), chosenDriver.getDriverId()));
                    ...
                    redisDriverTemplate
                        .opsForValue()
                        .set(MATCHING_DRIVER_CACHE_KEY, allMatchingCache);
                  }
                }
              }
            }
      ...
      if (!matched) {
        ...
        riderWaitingQueueCache.add(
            RiderWaitingQueueCache.builder()
                .riderId(riderId)
        ...
        redisWaitingQueueTemplate
            .opsForValue()
            .set(MATCHING_WAITING_QUEUE_KEY, riderWaitingQueueCache);
      }
    } catch (InvalidProtocolBufferException e) {
  ...
  public void cronJobMatchingAlgorithm() {
    ...
    try {
      ...
      // Run this CRON job every second to check whether there is a driver for the riders in the
      // waiting queue => pop the first element from the queue
      // Load caches from Redis (ensure initialization)
      HashMap<String, HashMap<String, List<MatchingDriverCache>>> allMatchingCache =
          ensureAllMatchingCache();
      ...
      if (pickUpStation != null && !pickUpStation.isEmpty()) {
        ...
        if (stationMap != null && !stationMap.isEmpty()) {
          ...
          for (String driverDestination : stationMap.keySet()) {
            ...
            // log.info("Checking driverDest: {}, riderDest: {}, distance: {}", driverDestination,
            // destinationPlace, distVal);
            ...
            if (distVal <= DISTANCE_THRESHOLD_UNITS) {
              ...
              if (driversAtDest != null) {
                for (MatchingDriverCache driverCache : driversAtDest) {
                  ...
                  //        driverCache.getDriverId(), driverArrivalMillis, riderMillis, diff,
                  // TIME_THRESHOLD_MS);
          ...
          if (!pq.isEmpty()) {
            ...
            if (chosenDriver != null) {
              ...
              CompletableFuture<SendResult<String, byte[]>> future =
                  kafkaTemplate.send(
                      MATCHING_TOPIC,
                      String.valueOf(event.getDriverId() + event.getRiderId()),
                      event.toByteArray());
              future
                  .thenAccept(
                      result -> {
