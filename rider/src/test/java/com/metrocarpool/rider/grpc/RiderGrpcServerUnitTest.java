...
class RiderGrpcServerUnitTest {
  ...
  @Mock private StreamObserver<RiderStatusResponse> responseObserver;
  ...
  void postRiderInfo_Success() {
    ...
    PostRider request =
        TestDataBuilder.buildPostRiderRequest(
            riderId, pickUpStation, destinationPlace, arrivalTime);
  ...
  void postRiderInfo_ReturnsSuccessStatus() {
    ...
    PostRider request =
        TestDataBuilder.buildPostRiderRequest(
            1L, "StationA", "LocationC", TestDataBuilder.buildTimestamp());
    when(riderService.processRiderInfo(anyLong(), anyString(), anyString(), any(Timestamp.class)))
        .thenReturn(true);

    ArgumentCaptor<RiderStatusResponse> responseCaptor =
        ArgumentCaptor.forClass(RiderStatusResponse.class);
  ...
  void postRiderInfo_CallsServiceWithCorrectParameters() {
    ...
    // Then
    verify(riderService)
        .processRiderInfo(eq(riderId), eq(pickUpStation), eq(destinationPlace), eq(arrivalTime));
  }
  ...
  void postRiderInfo_HandlesMultipleRequests() {
    ...
    PostRider request2 =
        TestDataBuilder.buildPostRiderRequest(
            2L, "StationB", "LocationD", TestDataBuilder.buildTimestamp());
    ...
    // Then
    verify(riderService, times(2))
        .processRiderInfo(anyLong(), anyString(), anyString(), any(Timestamp.class));
