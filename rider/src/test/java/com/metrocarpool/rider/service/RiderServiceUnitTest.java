...
class RiderServiceUnitTest {
  ...
  void processRiderInfo_UsesRiderIdAsKey() {
    ...
    CompletableFuture<SendResult<String, byte[]>> future = CompletableFuture.completedFuture(null);
    when(kafkaTemplate.send(anyString(), keyCaptor.capture(), any(byte[].class)))
        .thenReturn(future);
  ...
  void processRiderInfo_PublishesToCorrectTopic() {
    ...
    CompletableFuture<SendResult<String, byte[]>> future = CompletableFuture.completedFuture(null);
    when(kafkaTemplate.send(topicCaptor.capture(), anyString(), any(byte[].class)))
        .thenReturn(future);
  ...
  void processRiderInfo_GeneratesUniqueMessageIds() throws Exception {
    ...
    RiderRequestDriverEvent event1 =
        RiderRequestDriverEvent.parseFrom(eventCaptor.getAllValues().get(0));
    RiderRequestDriverEvent event2 =
        RiderRequestDriverEvent.parseFrom(eventCaptor.getAllValues().get(1));
