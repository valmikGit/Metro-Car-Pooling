...
public class RiderService {
   ...
   */
  public boolean processRiderInfo(
      Long riderId,
    ...
    try {
      ...
      RiderRequestDriverEvent riderRequestDriverEvent =
          RiderRequestDriverEvent.newBuilder()
      ...
      CompletableFuture<SendResult<String, byte[]>> future =
          kafkaTemplate.send(
              RIDER_TOPIC, riderId.toString(), riderRequestDriverEvent.toByteArray());
      future
          .thenAccept(
              result -> {
                log.debug(
                    "Event = {} delivered to {}",
                    riderRequestDriverEvent,
      ...
      log.info(
          "Published rider event for ID {} to topic '{}': {}",
          riderId,
          RIDER_TOPIC,
          riderRequestDriverEvent);
      return true;
