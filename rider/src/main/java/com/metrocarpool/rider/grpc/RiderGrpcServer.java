...
public class RiderGrpcServer extends RiderServiceGrpc.RiderServiceImplBase {
  ...
  @Override
  public void postRiderInfo(
      PostRider request, StreamObserver<RiderStatusResponse> responseObserver) {
    try {
      log.info("Reached RiderGrpcServer.postRiderInfo.");

      // ✅ Call the business logic
      boolean success =
          riderService.processRiderInfo(
              request.getRiderId(),
      ...
      // ✅ Build the response
      RiderStatusResponse response = RiderStatusResponse.newBuilder().setStatus(success).build();
