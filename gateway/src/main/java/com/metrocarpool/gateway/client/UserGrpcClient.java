...
public class UserGrpcClient {
  ...
  public SignUpOrLoginResponse DriverSignUpReq(DriverSignUpRequestDTO driverSignUpRequestDTO) {
    ...
    ServiceInstance instance =
        discoveryClient.getInstances("user").stream()
            .findFirst()
    ...
    try {
      ...
      return SignUpOrLoginResponse.newBuilder().setUserId(-1).setSTATUSCODE(401).build();
    } finally {
      channel.shutdown();
    }
  }
  ...
  public SignUpOrLoginResponse RiderSignUpReq(RiderSignUpRequestDTO riderSignUpRequestDTO) {
    ...
    try {
      ...
      return SignUpOrLoginResponse.newBuilder().setSTATUSCODE(401).setUserId(-1).build();
    } finally {
      channel.shutdown();
    }
  }
