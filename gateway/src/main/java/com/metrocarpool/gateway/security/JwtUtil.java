...
/**
 * Utility class for creating, validating, and extracting claims from JWT tokens. It uses the
 * SECRET_KEY defined in JwtConstant for signing and verification.
 */
@Component
@Slf4j
public class JwtUtil {
  ...
  /**
   * Initializes the signing key using the secret string. This runs once after the bean is
   * constructed.
   */
  @PostConstruct
  public void init() {
  ...
  /**
   * Generates a new JWT token signed with the configured secret key. This token should be returned
  ...
  /**
   * Validates the integrity and expiration of the given JWT token. Note: In a Gateway scenario,
   * this method is used by the JwtGlobalFilter.
   *
