...
public class NotificationService {
  ...
  private final Sinks.Many<DriverRideCompletion> driverCompletionSink =
      Sinks.many().replay().latest();
  private final Sinks.Many<RiderRideCompletion> riderCompletionSink =
      Sinks.many().replay().latest();
  private final Sinks.Many<NotifyRiderDriverLocation> driverLocationForRiderSink =
      Sinks.many().replay().latest();
  ...
  private static final String DRIVER_RIDE_COMPLETION_KAFKA_DEDUP_KEY_PREFIX =
      "driver_rider_completion_processed_kafka_msg:";
  private static final String RIDER_RIDE_COMPLETION_KAFKA_DEDUP_KEY_PREFIX =
      "rider_rider_completion_processed_kafka_msg:";
  ...
  public void publishRiderDriverMatch(byte[] message, Acknowledgment ack) {
    try {
      ...
      RiderDriverMatch match =
          RiderDriverMatch.newBuilder()
              .setRiderId(riderId)
      ...
      if (alreadyProcessed(RIDER_DRIVER_MATCH_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "NotificationService.publishRiderDriverMatch: Duplicate Kafka message detected."
                + " Skipping. messageId={}",
            messageId);
        ack.acknowledge();
        return;
  ...
  // 🧠 This will be called by your Kafka listener whenever a new driver ride completion event
  // arrives.
  @KafkaListener(
      topics = "${kafka.topics.driver-ride-completion}",
      groupId = "${spring.kafka.consumer.group-id}")
  public void publishDriverRideCompletion(byte[] byteMessage, Acknowledgment ack) {
    try {
      ...
      DriverRideCompletion completion =
          DriverRideCompletion.newBuilder()
              .setDriverId(driverId)
      ...
      if (alreadyProcessed(DRIVER_RIDE_COMPLETION_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "NotificationService.publishDriverRideCompletion: Duplicate Kafka message detected."
                + " Skipping. messageId={}",
            messageId);
        ack.acknowledge();
        return;
  ...
  // 🧠 This will be called by your Kafka listener whenever a new rider ride completion event
  // arrives.
  @KafkaListener(
      topics = "${kafka.topics.rider-ride-completion}",
      groupId = "${spring.kafka.consumer.group-id}")
  public void publishRiderRideCompletion(byte[] byteMessage, Acknowledgment ack) {
    try {
      ...
      long riderId = tempEvent.getRiderId();
      String message = tempEvent.getEventMessage();
      RiderRideCompletion completion =
          RiderRideCompletion.newBuilder()
              .setRiderId(riderId)
              .setCompletionMessage(message)
              .build();
      ...
      if (alreadyProcessed(RIDER_RIDE_COMPLETION_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "NotificationService.publishRiderRideCompletion: Duplicate Kafka message detected."
                + " Skipping. messageId={}",
            messageId);
        ack.acknowledge();
        return;
  ...
  @KafkaListener(
      topics = "${kafka.topics.driver-location-rider}",
      groupId = "${spring.kafka.consumer.group-id}")
  public void publishDriverLocationForRiderEvent(byte[] byteMessage, Acknowledgment ack) {
    try {
      ...
      DriverLocationForRiderEvent driverLocationForRiderEvent =
          DriverLocationForRiderEvent.parseFrom(byteMessage);
      String messageId = driverLocationForRiderEvent.getMessageId();

      NotifyRiderDriverLocation notifyRiderDriverLocation =
          NotifyRiderDriverLocation.newBuilder()
      ...
      if (alreadyProcessed(DRIVER_LOCATION_RIDER_KAFKA_DEDUP_KEY_PREFIX, messageId)) {
        log.info(
            "NotificationService.driverLocationForRiderEvent: Duplicate Kafka message detected."
                + " Skipping. messageId={}",
            messageId);
        ack.acknowledge();
        return;
